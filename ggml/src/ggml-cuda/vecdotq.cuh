...
// GCN fast paths via memcpy (emits flat_load) and v_perm for MXFP4

static __device__ __forceinline__ int ggml_gcn_get_int_b1_fast(const void * x, const int & i32) {
    int x32;
    memcpy(&x32, (const uint8_t *) x + 4*i32, 4);
    return x32;
}

static __device__ __forceinline__ int ggml_gcn_get_int_b2_fast(const void * x, const int & i32) {
    int x32;
    memcpy(&x32, (const uint8_t *) x + 4*i32, 4);
    return x32;
}

// MXFP4 8-entry magnitude LUT (0,1,2,3,4,6,8,12) with sign extension via v_perm
__constant__ uint8_t ggml_gcn_mxfp4_magnitudes[8] = { 0, 1, 2, 3, 4, 6, 8, 12 };

static __device__ __forceinline__ int2 ggml_gcn_get_int_from_mxfp4_table(const uint32_t q4) {
    const uint32_t * mags32 = (const uint32_t *) ggml_gcn_mxfp4_magnitudes;
...
#define GGML_GCN_VEC_DOT_MXFP4_Q8_1(bq4, bq8_1, iqs, sumi) \
    do { \
        const int * q8 = (const int *) bq8_1->qs + iqs; \
        const int aux_q4_0 = ggml_gcn_get_int_b1_fast(bq4->qs, iqs + 0); \
        const int aux_q4_1 = ggml_gcn_get_int_b1_fast(bq4->qs, iqs + 1); \
        const int2 v0 = ggml_gcn_get_int_from_mxfp4_table(aux_q4_0); \
        const int2 v1 = ggml_gcn_get_int_from_mxfp4_table(aux_q4_1); \
...
#if defined(GGML_USE_HIP) && defined(GCN)
    int sumi = 0;
    GGML_GCN_VEC_DOT_MXFP4_Q8_1(bq4, bq8_1, iqs, sumi);
#else
    const int * q8 = (const int *) bq8_1->qs + iqs;
...
#if defined(GGML_USE_HIP) && defined(GCN)
        v[i] = ggml_gcn_get_int_b2_fast(bq8_0->qs, iqs + i);
#else
